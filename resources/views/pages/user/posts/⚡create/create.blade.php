<main class="postsCreate">
    <x-user.utils.heading title="Ajouter une annonce" text="Publiez votre matériel médical en quelques étapes pour lui donner une seconde vie"/>
            ...
            <form class="formContainer__form" wire:submit="store()" enctype="multipart/form-data" method="post">
                ...
                <fieldset class="formContainer__form__fieldset">
                    ...
                    <x-user.form.fields.select wire:model="form.type" name_parent="formContainer__form__fieldset" field_name="post_type" required="required" label="Type d'annonce">
                        <x-user.form.fields.option value="none" option_name="Sélectionnez le type" name_parent="formContainer__form__fieldset" selected="selected"/>
                        @foreach($this->getType() as $type)
                            <x-user.form.fields.option value="{!! $type->name !!}" option_name="{!! $type->value !!}" name_parent="formContainer__form__fieldset"/>
                        @endforeach
                    </x-user.form.fields.select>
                    ...
                    <x-user.form.fields.select wire:model="form.state" name_parent="formContainer__form__fieldset" field_name="post_state" required="required" label="État du produit">
                        <x-user.form.fields.option value="none" option_name="Sélectionnez l’état" name_parent="formContainer__form__fieldset" selected="selected"/>
                        @foreach($this->getState() as $state)
                            <x-user.form.fields.option value="{!! $state->name !!}" option_name="{!! $state->value !!}" name_parent="formContainer__form__fieldset"/>
                        @endforeach
                    </x-user.form.fields.select>
                </fieldset>
